...
import (
	...
	"github.com/dwnGnL/testWork/internal/handler/private"
...
func SetupHandlers(core application.Core, cfg *config.Config) GracefulStopFuncWithCtx {
	...
	private.Use(application.CheckBaererToken(core))
	generatePrivateRouting(private, cfg)
...
func generatePrivateRouting(gE *gin.RouterGroup, cfg *config.Config) {
	private.GenRouting(gE, cfg)
}
